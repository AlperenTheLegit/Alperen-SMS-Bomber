...
class SendSms():
    ...
    def Kigili(self):    
        try:
            kigili = requests.post("https://uyelik.chp.org.tr/Default.aspx", data={
                "first_name": "Memati",
                "last_name": "Bas",
                "email": f"{self.random_mail}@gmail.com",
                "phone": f"0{self.phone}",
            ...
            if kigili.status_code == 202:
                print(f"{Fore.LIGHTGREEN_EX}[+] {Style.RESET_ALL}Başarılı! --> chp.org")
                self.adet += 1
            else:
                raise
        except:
            print(f"{Fore.LIGHTRED_EX}[-] {Style.RESET_ALL}Başarısız! --> chp.org")
